...
- **Read-only responder.** Claude Code runs with only `Read`, `Grep`, and
  `Glob`; write, shell, subagent, and web tools are blocked. Codex runs with
  `--sandbox read-only`.
...
- **The responder can read any file your user can read.** Its read tools are
  not confined to the project. A transcript containing a prompt injection
  could steer it to read a file outside the project and quote it in an
  answer. That answer goes to your model service, as your agent's own
  traffic does, and is shown in your local viewer.
