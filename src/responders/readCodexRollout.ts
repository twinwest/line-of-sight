...
import { readCompressedCodex } from '../adapters/codexRollout.js';

try {
  const filePath = process.argv[2];
  if (!filePath?.endsWith('.jsonl.zst')) throw new Error('Expected a compressed .jsonl.zst rollout path');
  await readCompressedCodex(filePath, async lines => {
    if (!process.stdout.write(lines.map(line => line.text + '\n').join(''))) await once(process.stdout, 'drain');
  });
} catch (e) {
