...
| B9 | **Mirror the disk, don't archive it** | Sight renders the transcripts on disk and nothing else. When one goes away (the CLI's own retention cleanup, an `rm`), its session leaves Sight — subagent runs, side chats and all. No archive, no soft delete, no retention of our own. Opt-in exception: `keepSideChats` keeps the user's side chats — question, answers, and the conversation snapshot taken at ask time — after the transcript is gone. |
